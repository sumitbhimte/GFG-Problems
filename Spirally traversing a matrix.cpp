...
op = 1 5 9 13 14 15 16 12 8 4 3 2 6 10 11 7
...
{   
    ...
    vector<int> spirallyTraverse(vector<vector<int> > matrix, int r, int c) 
    {
        ...
        while(top<bottom and left<right){
            ...
            for(int i=top;i<bottom;i++)  v.push_back(matrix[i][left]);
            left++;
            
            
            for(int i=left;i<right;i++)  v.push_back(matrix[bottom-1][i]);
            bottom--;
            
            if(left<right){
                
                for(int i=bottom-1;i>=top;i--)  v.push_back(matrix[i][right-1]);
                right--;
            }
            
            if(top<bottom){
                
                for(int i=right-1;i>=left;i--)  v.push_back(matrix[top][i]);
                top++;
            }
